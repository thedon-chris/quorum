...
contract BlockVoting {
	...
	function setVoteThreshold(uint threshold) public mustBeVoter {
	...
	function vote(uint height, bytes32 hash) public mustBeVoter {
		...
		Period storage period = periods[height-1];
	...
	function getCanonHash(uint height) public constant returns(bytes32) {
		Period storage period = periods[height-1];
 		bytes32 best;
	...
	function addVoter(address addr) public mustBeVoter {
		if (!canVote[addr]) {
		    canVote[addr] = true;
		    voterCount++;
		    AddVoter(addr);
	...
	function isVoter(address addr) public constant returns (bool) {
	    return canVote[addr];
	}
     // addBlockMaker adds the given list to the collection of addresses that
    // are allowed to create blocks.
	function addBlockMaker(address addr) public mustBeGovernance {
	...
	function removeBlockMaker(address addr) public mustBeGovernance {
		require(blockMakerCount > 1);
 	    if (canCreateBlocks[addr]) {
	...
	function isBlockMaker(address addr) public constant returns (bool) {
	...
	function getEntry(uint height, uint n) public constant returns(bytes32) {
		Period storage period = periods[height-1];
